...
func main() {
	...
	loser, number := getLoser(calledNumbers, boards)
	score := processScore(loser, number)
...
func processScore(winner []int, lastNumber int) int {
	score := 0
...
func getLoser(numbers []int, boards [][]int) ([]int, int) {
	lastWinner := make([]int, 0)
	lastNumber := -1
out:
	for _, n := range numbers {
		for b := len(boards) - 1; b >= 0; b-- {
			board := boards[b]
				...
				if n == v {
					...
					if checkForWinner(board) == true {
						if len(boards) > 1 {
							...
							lastWinner = append(lastWinner, board...)
							lastNumber = n
							break out
						}
					}
